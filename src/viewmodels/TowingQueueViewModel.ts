...
import { updateTowingQueueItem, removeFromTowingQueue } from '../redux/slices/towingQueueSlice';
import { TowingQueueItem } from '../models/TowingQueue';

export const useTowingQueueViewModel = () => {
  const dispatch = useDispatch();
  const { items, loading } = useSelector((state: RootState) => state.towingQueue);
