...
import { checkVehicleRequest, clearVehicle } from '../redux/slices/vehicleSlice';
import { addToTowingQueue } from '../redux/slices/towingQueueSlice';
import { Property } from '../models/Property';

export const useScanViewModel = () => {
  const dispatch = useDispatch();
  const { properties, selectedProperty } = useSelector((state: RootState) => state.property);
  const { currentVehicle, loading } = useSelector((state: RootState) => state.vehicle);
  ...
  const checkVehicle = (licensePlate: string, parkingSpot: string) => {
    if (selectedProperty) {
      dispatch(checkVehicleRequest({ licensePlate, parkingSpot, propertyId: selectedProperty.id }));
    }
  };

  const sendToTowingQueue = () => {
    if (currentVehicle && selectedProperty) {
      dispatch(addToTowingQueue({
