...
const ScanTabContent: React.FC<ScanTabContentProps> = ({
  ...
  const handleStartScan = () => {
    ...
    if (!device) {
      Alert.alert('Camera Not Available', 'No camera device found.', [
        { text: 'OK' },
      ]);
      return;
    }
  ...
  const takePicture = async () => {
    ...
    try {
      ...
      if (result.results && result.results.length > 0) {
        ...
        const dummyParkingSpot =
          parkingSpot || `A-${Math.floor(Math.random() * 200) + 1}`;
  ...
  return !isScanning
    ? renderNotScanningSegment()
    : isScanning && device && hasPermission && Camera
    ? renderScanningSegment()
    : renderNotHavingPermissionContent();
};
